...
6. **Analyze the Issue:** As per the memory `palette.md` guidelines for UX and accessibility:
...
7. **Verify Code:** In `client/src/pages/Home.tsx`, the labels for "Data e Hora do Voo", "Número do Voo", and "Localizador (PNR)" (both Departure and Return) have `htmlFor` attributes, but the corresponding `<input>` elements are missing the matching `id` attributes.
...
8. **Plan Steps:** - Modify `client/src/pages/Home.tsx` to add the missing `id` attributes to the 6 inputs mentioned above. - Run tests/lint to verify. - Create PR following Palette guidelines.
