...
## 2024-05-24 - Missing Form Label Associations in Flight Quotes
**Learning:** Discovered that manual input fields within the FlightQuotes week cards lacked programmatic associations (missing `id` and `htmlFor`), making them inaccessible to screen readers because they aren't wrapped directly in the `<label>`.
**Action:** Always explicitly link descriptive `<label>` elements to their corresponding `<input>` components using unique `htmlFor` and `id` attributes, especially when rendering lists or using custom UI components like `Input`.
