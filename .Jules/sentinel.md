...
## 2026-07-27 - tRPC Error Formatting
**Vulnerability:** Information Leakage on Server Errors
**Learning:** Default tRPC configurations can inadvertently leak internal stack traces or database details to the client when an unhandled `INTERNAL_SERVER_ERROR` occurs.
**Prevention:** Configure a custom `errorFormatter` in the `initTRPC` setup to mask sensitive error details with generic messages in production environments.
