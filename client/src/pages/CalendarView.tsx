...
import { ArrowLeft, Plane, Circle, X, MessageCircle, CalendarPlus, Download, ExternalLink } from 'lucide-react';
import {
...
function FlightPopup({
  ...
  const depAirlineName = airlineNames[(w.departureAirline ?? '').toLowerCase()] ?? w.departureAirline ?? '—';
  const retAirlineName = airlineNames[(w.returnAirline ?? '').toLowerCase()] ?? w.returnAirline ?? '—';
  ...
  const depEvent: CalendarEventParams | null = w.departureFlightDatetime
    ? {
        ...
        location: airportAddresses[depAirport] ?? airportNames[depAirport] ?? depAirport,
        description: [
          `Semana ${w.weekNumber}`,
  ...
  const retEvent: CalendarEventParams | null = (!isOneway && w.returnFlightDatetime)
    ? {
        title: `✈️ Voo VOLTA ${w.returnFlightNumber ?? ''} — ${retAirlineName}`,
        flightDatetime: w.returnFlightDatetime,
        location: airportAddresses[retAirport] ?? airportNames[retAirport] ?? retAirport,
        description: [
          `Semana ${w.weekNumber}`,
  ...
  // Rastreamento
  const depTrackUrl = (w.departureFlightNumber && depIsoDate)
    ? buildFlightTrackUrl(w.departureAirline ?? '', w.departureFlightNumber, depAirport, retAirport, depIsoDate + 'T00:00')
    : null;
  const retTrackUrl = (!isOneway && w.returnFlightNumber && retIsoDate)
    ? buildFlightTrackUrl(w.returnAirline ?? '', w.returnFlightNumber, retAirport, depAirport, retIsoDate + 'T00:00')
    : null;
      ...
      >
        ...
        <div className="bg-blue-700 px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-bold text-sm">Semana {w.weekNumber}</span>
            {isOneway && (
        ...
        <div className="p-4 flex flex-col gap-3">
          ...
          <div className="rounded-xl border border-blue-100 bg-blue-50 overflow-hidden">
            <div className="bg-blue-600 px-3 py-1.5 flex items-center gap-1.5">
              ...
              <span className="text-[11px] font-bold text-white uppercase tracking-wider">Ida</span>
              <span className="ml-auto text-[10px] text-blue-200">{depAirport} → {retAirport}</span>
            </div>
            <div className="px-3 py-2 grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs">
              <div>
                <span className="text-[9px] font-semibold text-blue-400 uppercase tracking-wide block">Data</span>
                <span className="text-slate-700 font-medium">{w.departureDate || '—'}</span>
              </div>
              <div>
                <span className="text-[9px] font-semibold text-blue-400 uppercase tracking-wide block">Horário</span>
                <span className="text-slate-700 font-medium">{depTime || '—'}</span>
              </div>
              <div>
                <span className="text-[9px] font-semibold text-blue-400 uppercase tracking-wide block">Companhia</span>
                <span className="text-slate-700 font-medium">{depAirlineName}</span>
              </div>
              <div>
                <span className="text-[9px] font-semibold text-blue-400 uppercase tracking-wide block">Voo</span>
                <span className="text-slate-700 font-mono font-medium">{w.departureFlightNumber || '—'}</span>
              </div>
              {w.departureLocator && (
                <div className="col-span-2">
                  <span className="text-[9px] font-semibold text-blue-400 uppercase tracking-wide block">Localizador</span>
                  <span className="text-slate-700 font-mono font-bold tracking-widest">{w.departureLocator}</span>
                </div>
              )}
              {depTrackUrl && (
                <div className="col-span-2">
          ...
          {!isOneway && (
            <div className="rounded-xl border border-orange-100 bg-orange-50 overflow-hidden">
              <div className="bg-orange-500 px-3 py-1.5 flex items-center gap-1.5">
                ...
                <span className="text-[11px] font-bold text-white uppercase tracking-wider">Volta</span>
                <span className="ml-auto text-[10px] text-orange-200">{retAirport} → {depAirport}</span>
              </div>
              <div className="px-3 py-2 grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs">
                <div>
                  <span className="text-[9px] font-semibold text-orange-400 uppercase tracking-wide block">Data</span>
                  <span className="text-slate-700 font-medium">{w.returnDate || '—'}</span>
                </div>
                <div>
                  <span className="text-[9px] font-semibold text-orange-400 uppercase tracking-wide block">Horário</span>
                  <span className="text-slate-700 font-medium">{retTime || '—'}</span>
                </div>
                <div>
                  <span className="text-[9px] font-semibold text-orange-400 uppercase tracking-wide block">Companhia</span>
                  <span className="text-slate-700 font-medium">{retAirlineName}</span>
                </div>
                <div>
                  <span className="text-[9px] font-semibold text-orange-400 uppercase tracking-wide block">Voo</span>
                  <span className="text-slate-700 font-mono font-medium">{w.returnFlightNumber || '—'}</span>
                </div>
                {w.returnLocator && (
                  <div className="col-span-2">
                    <span className="text-[9px] font-semibold text-orange-400 uppercase tracking-wide block">Localizador</span>
                    <span className="text-slate-700 font-mono font-bold tracking-widest">{w.returnLocator}</span>
                  </div>
                )}
                {retTrackUrl && (
                  <div className="col-span-2">
          ...
          <div className="border-t border-slate-100 pt-3 flex flex-col gap-2">
            <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Compartilhar & Agenda</span>
            ...
            {allEvents.length > 0 && (
              <div className="grid grid-cols-2 gap-2">
                ...
                {/* Download ICS */}
                <button
                  onClick={() => downloadICS(allEvents, `voo-semana-${w.weekNumber}.ics`)}
                  className="col-span-2 flex items-center justify-center gap-1.5 px-2 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 transition-colors text-slate-700 text-[11px] font-semibold border border-slate-200"
                >
...
export default function CalendarView() {
  ...
  const markedDays = useMemo<Record<string, DayMark>>(() => {
    ...
    for (const week of weeksQuery.data as WeekRow[]) {
      ...
      if (depDate) {
        ...
        if (!map[key]) map[key] = { departure: false, return: false, isPast, week };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* Popup flutuante */}
      {selectedMark && (
        <FlightPopup mark={selectedMark} onClose={() => setSelectedMark(null)} />
      )}
                  ...
                  {grid.map((week, wi) => (
                    <div key={wi} className="grid grid-cols-7 gap-0.5">
                      {week.map((day, di) => {
                        ...
                        if (mark) {
                          cellBg = mark.isPast ? 'bg-slate-500' : 'bg-emerald-500';
                        ...
                        return (
                          <div
                            key={di}
                            title={mark ? `Semana ${mark.week.weekNumber} — clique para detalhes` : holiday || undefined}
          ...
          {weeksQuery.isLoading ? (
            ...
            <>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-slate-700">
                  {(weeksQuery.data as WeekRow[] | undefined)?.filter(w => !w.isTicketIssued && !(w as any).isDeleted).length ?? 0} semanas sem bilhete
                </span>
              </div>
            </>
          )}
