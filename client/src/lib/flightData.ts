...
// ⚡ Bolt: Pre-compute the timestamps for the holidays array to avoid redundant parsing inside the render loop
const feriados2026Parsed = feriados2026.map(f => ({
  feriado: f,
  time: parseDate(f.data).getTime(),
}));
...
export function getFeriadosDaSemana(
  ...
  for (let i = 0; i < feriados2026Parsed.length; i++) {
    const { feriado: f, time: fTime } = feriados2026Parsed[i];
