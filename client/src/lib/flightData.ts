...
// ⚡ Bolt: Conversão rápida de data DD/MM/YYYY para inteiro YYYYMMDD para comparação rápida
function parseDateStrToInt(dateStr: string): number {
  const parts = dateStr.split("/");
  return (
    parseInt(parts[2], 10) * 10000 +
    parseInt(parts[1], 10) * 100 +
    parseInt(parts[0], 10)
  );
}

// ⚡ Bolt: Pré-computa as datas dos feriados em inteiros para evitar parsing no render
const parsedFeriados = feriados2026.map(f => ({
  feriado: f,
  dateInt: parseDateStrToInt(f.data),
}));
...
export function getFeriadosDaSemana(
  ...
  const depInt = parseDateStrToInt(departureDate);
  const retInt = parseDateStrToInt(returnDate);

  // ⚡ Bolt: Loop otimizado comparando apenas inteiros, sem criar objetos Date
  for (let i = 0; i < parsedFeriados.length; i++) {
    const f = parsedFeriados[i];
    const fInt = f.dateInt;

    if (fInt === depInt) {
      result.push({ feriado: f.feriado, tipo: "ida" });
    } else if (fInt === retInt) {
      result.push({ feriado: f.feriado, tipo: "retorno" });
    } else if (fInt > depInt && fInt < retInt) {
      result.push({ feriado: f.feriado, tipo: "intervalo" });
    }
  }
  return result;
}
