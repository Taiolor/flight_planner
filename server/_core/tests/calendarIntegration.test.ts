...
import { createFlightCalendarEvent, createRoundTripCalendarEvents, FlightEvent } from '../calendarIntegration';
...
describe('calendarIntegration', () => {
  ...
  describe('createFlightCalendarEvent', () => {
    it('should create an event successfully with correct parameters', async () => {
      vi.mocked(childProcess.execFileSync).mockReturnValue(JSON.stringify({ success: true }));
      ...
      expect(calEvent.description).toContain('https://www.google.com/search?q=LA+flight+1234+from+GRU+to+SDU,+2024-10-15');
    });
    ...
    it('should format URL correctly with one-digit day/month and short flight numbers', async () => {
      ...
      expect(calEvent.description).toContain('https://www.google.com/search?q=AZ+flight+0042+from+VCP+to+CNF,+2024-01-05');
    ...
    it('should use airport addresses if provided', async () => {
      ...
      const calEvent = payload.events[0];
      expect(calEvent.location).toBe('Viracopos Airport, Campinas → Confins Airport, BH');
    });
  });
  ...
  describe('createRoundTripCalendarEvents', () => {
    it('should create both departure and return events', async () => {
      ...
      const payload1 = JSON.parse(inputArgs1[inputArgs1.indexOf('--input') + 1]);
      const payload2 = JSON.parse(inputArgs2[inputArgs2.indexOf('--input') + 1]);
